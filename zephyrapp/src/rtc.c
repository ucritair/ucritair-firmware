...
static int board_cat_init_rtc(void)
{
    ...
    nrfx_coredep_delay_us(1000*62); // as of SEA-1 we are 1.563s - so 13 ticks (1.625) forward and then 62ms delay to get on time
...
{
	rtc_offset = rtc_offset + (uint64_t)nrf_rtc_counter_get(HW_RTC_CHOSEN) + 13; // as of SEA-1 we are 1.563s - so 13 ticks (1.625) forward and then 62ms delay to get on time
} 
