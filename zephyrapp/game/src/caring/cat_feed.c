	...
	if(active_food_count > 1)
	{
		...
		for(int i = 0; i < food_count; i++)
		{
			...
			for(int j = 0; j < food_count; j++)
			{
				...
				float best_angle =
				best_neighbour == -1 ?
				INFINITY : food_list[best_neighbour].angle;
...
float score_spacing()
{
	...
	return 
	active_food_count > 0 ?
	1.0f - (float) collision_count / (float) active_food_count :
	0;
}
	...
	for(int i = food_count-1; i >= 0; i--)
	{
		...
		if(show_gizmos)
		{
			...
			if(food_list[i].active)
			{
				...
				uint16_t evenness_colour = CAT_RGB8882565(CAT_RGB888_lerp(red, green, score_object.evenness));
