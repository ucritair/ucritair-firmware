...
void CAT_render_debug()
{
	switch(page)
	{
		case SYSTEM:
		{
			...
			cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "[Desktop]\n");
#endif
#if defined(CAT_EMBEDDED)
			cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "[Embedded]\n");
#endif
#if CAT_PRIORITIZE_AQ
			cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "[AQ First]\n");
#endif
#if CAT_RADIO_ENABLED
			cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "[Radio Enabled]\n");
#endif
#if CAT_WIFI_ENABLED
		...
		case TIME:
		{
			...
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Since Life: %llus/%ds\n", now-pet_timing_state.last_life_time, CAT_LIFE_TICK_PERIOD);
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Since Stat: %llus/%ds\n", now-pet_timing_state.last_stat_time, CAT_STAT_TICK_PERIOD);

			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Slept: %llus\n", CAT_get_slept_s());
		}
		break;
		...
		case AQI:
		{
			...
			if(CAT_AQ_sensors_initialized())
				cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "Sensors initialized\n");

			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "RH: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(readings.sen5x.humidity_rhpct));
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "CO2: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(readings.sunrise.ppm_filtered_compensated));
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "PM: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(readings.sen5x.pm2_5));
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "NOX: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(readings.sen5x.nox_index));
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "VOC: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(readings.sen5x.voc_index));
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "TMP: " CAT_FLOAT_FMT "%s\n", CAT_FMT_FLOAT(CAT_AQ_map_celsius(readings.lps22hh.temp)), CAT_AQ_get_temperature_unit_string());
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "AQI: " CAT_FLOAT_FMT "\n", CAT_FMT_FLOAT(CAT_AQ_aggregate_score()));
		}
		break;
		...
		case LOGS:
		{
			...
			if(CAT_AQ_sensors_initialized())
				cursor_y = CAT_draw_text_depr(cursor_x, cursor_y, "Logs initialized\n");

			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Log count: %d\n", CAT_get_log_cell_count());
			
			CAT_log_cell cell;
			int cell_idx = CAT_read_first_calendar_cell(&cell);
			cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Earliest log: %d\n", cell_idx);
		...
		case NETWORK:
		{
			...
			{
				CAT_set_text_mask_depr(PAD, -1, CAT_LCD_SCREEN_W-12, -1);
				CAT_set_text_flags_depr(CAT_TEXT_FLAG_WRAP);
				cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Connected to WiFi network %s\n", CAT_get_wifi_SSID());
			}
			else
			{
				CAT_set_text_mask_depr(PAD, -1, CAT_LCD_SCREEN_W-12, -1);
				CAT_set_text_flags_depr(CAT_TEXT_FLAG_WRAP);
				cursor_y = CAT_draw_textf_depr(cursor_x, cursor_y, "Device is not connected to WiFi\n");
			}
		}
		break;
